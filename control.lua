   ...
   local function on_active( entity )
   local logistic19 = entity.surface.create_entity{name="logistic-chest-active-provider-1", position=entity.position, force=entity.force}
   logistic19.destructible = true
   logistic19.minable = true
   
   local invipole32 = entity.surface.create_entity{name="invisible-electric-pole", position=entity.position, force=entity.force}
   invipole32.destructible = false
   invipole32.minable = false
   return true
   end
...
script.on_event(defines.events.on_built_entity, function(event)
      if event.created_entity.name == "logistic-mining-drill" then
      on_active( event.created_entity ) end	  
...
script.on_event(defines.events.on_player_mined_entity, function(event)
    if event.entity.name == "logistic-mining-drill" then
    ...
    name = "logistic-chest-active-provider"}) do
    entity.destroy()    
    end
    for _, entity in pairs(event.entity.surface.find_entities_filtered{
    area = {{center.x-0.5, center.y-0.5}, {center.x+0.5, center.y+0.5}},
...
script.on_event(defines.events.on_built_entity, function(event)
      if event.created_entity.name == "logistic-assembling-machine" then
      on_ass( event.created_entity ) end	  
...
script.on_event(defines.events.on_player_mined_entity, function(event)
    if event.entity.name == "logistic-assembling-machine" then
    center = event.entity.position
	for _, entity in pairs(event.entity.surface.find_entities_filtered{
    area = {{center.x-1.2, center.y-1.2}, {center.x+1.2, center.y+1.2}},
    name = "invisible-inserter"}) do
    entity.destroy()
    end 
	for _, entity in pairs(event.entity.surface.find_entities_filtered{
    area = {{center.x-1.2, center.y-1.2}, {center.x+1.2, center.y+1.2}},
    name = "invisible-inserter-2"}) do
    entity.destroy()
    end 
	for _, entity in pairs(event.entity.surface.find_entities_filtered{
    area = {{center.x-1.2, center.y-1.2}, {center.x+1.2, center.y+1.2}},
    name = "invisible-substation"}) do
    entity.destroy()
    end 
	end	
end
